...
//`feature_4`, `feature_5`, `feature_6`SELECT * FROM `product_details` WHERE 1
 ...
 <form method="post" action="PHP/techprocess.php">
 

    <input type="text" name="amount" value="<?php echo $sub_total*1;?>.0">
    <input type="text" name="mail_id" value="<?php echo $session_user_name;?>">
		<?php
		//`user_id`, `name`, `street`, `city`, `country`, `postcode`, `mobile_no`, `user_name`, `password`SELECT * FROM `users` WHERE 1
		$query41=mysqli_query($conn,"select * from users where user_name='$session_user_name'");
              if($f41=mysqli_fetch_array($query41)) 
               {
				  ?>
	 <input type="text" name="user_id" value="<?php echo $f41['user_id'];?>">
	 <input type="text" name="cust_name" value="<?php echo $f41['name'];?>">
	 <?php
			   }
				  
    //`trans_no`SELECT * FROM `trans_no_generator` WHERE 1
       $query6=mysqli_query($conn,"select * from trans_no_generator");
              if($f6=mysqli_fetch_array($query6)) 
               {?>
        <input type="text" name="trans_no" value="<?php echo $f6['trans_no'];?>">      
        <?php
           }
        ?>  
